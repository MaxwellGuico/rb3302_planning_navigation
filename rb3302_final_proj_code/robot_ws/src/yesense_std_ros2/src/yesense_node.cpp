...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	{
		pub_imu_ros 		= this->create_publisher<sensor_msgs::msg::Imu>("imu_data_ros", rclcpp::SensorDataQoS());
		...
		*((unsigned int *)&yis_out) = 0u;		
		...
		this->declare_parameter<bool>("publish_extended_topics", false);
		this->declare_parameter<double>("orientation_stddev", 0.20);
		this->declare_parameter<double>("angular_velocity_stddev", 0.03);
		this->declare_parameter<double>("linear_acceleration_stddev", 0.35);
		...
		this->get_parameter("publish_extended_topics", publish_extended_topics);
		this->get_parameter("orientation_stddev", orientation_stddev);
		this->get_parameter("angular_velocity_stddev", angular_velocity_stddev);
		this->get_parameter("linear_acceleration_stddev", linear_acceleration_stddev);
		...
		RCLCPP_INFO(this->get_logger(), "publish extended topics: %s\n",
			publish_extended_topics ? "true" : "false");
	...
	void timer_callback()
	{
		...
		{	
			...
			if(available > 0u)
			{
				const size_t bytes_to_read =
					available < UART_RX_BUF_LEN ? available : UART_RX_BUF_LEN;
		...
		// One serial read can contain several complete 200 Hz packets. Drain them
		// all now instead of leaving samples queued until more bytes arrive.
		for(unsigned int decoded = 0u; analysis_ok == ret && decoded < 64u; decoded++)
		{
			if(yis_out.content.valid_flg)
			{
				...
				RCLCPP_DEBUG_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
							"msg rate: %d, tid %d, acc: %f, %f, %f gyro: %f, %f, %f, euler: %f, %f, %f!", 
							user_info.msg_rate,
	...
	bool publish_extended_topics = false;
	double orientation_stddev = 0.20;
	double angular_velocity_stddev = 0.03;
	double linear_acceleration_stddev = 0.35;
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
    ...
    if(result->content.quat)
    {
        ...
        if(norm > 1e-6)
        {
            // YESENSE wire order is scalar-first q0,q1,q2,q3; ROS is x,y,z,w.
            imu_ros_data.orientation.x = result->quat.q1 / norm;
            imu_ros_data.orientation.y = result->quat.q2 / norm;
            imu_ros_data.orientation.z = result->quat.q3 / norm;
            imu_ros_data.orientation.w = result->quat.q0 / norm;
            const double variance = orientation_stddev * orientation_stddev;
            imu_ros_data.orientation_covariance[0] = variance;
            imu_ros_data.orientation_covariance[4] = variance;
            imu_ros_data.orientation_covariance[8] = variance;
        }
        else
        {
            imu_ros_data.orientation.w = 1.0;
    ...
    // Decoder data id 0x20 is degrees/s; sensor_msgs/Imu requires rad/s.
    imu_ros_data.angular_velocity.x = result->gyro.x * DEG_TO_RAD;
    imu_ros_data.angular_velocity.y = result->gyro.y * DEG_TO_RAD;
    imu_ros_data.angular_velocity.z = result->gyro.z * DEG_TO_RAD;
    ...
    imu_ros_data.angular_velocity_covariance[0] = angular_variance;
    imu_ros_data.angular_velocity_covariance[4] = angular_variance;
    imu_ros_data.angular_velocity_covariance[8] = angular_variance;

    const double acceleration_variance =
        linear_acceleration_stddev * linear_acceleration_stddev;
    imu_ros_data.linear_acceleration_covariance[0] = acceleration_variance;
    imu_ros_data.linear_acceleration_covariance[4] = acceleration_variance;
    imu_ros_data.linear_acceleration_covariance[8] = acceleration_variance;
    if(result->content.acc && result->content.gyro)
    {
        pub_imu_ros->publish(imu_ros_data);
    }

    // The standard ROS IMU topic is all Cartographer needs. Constructing and
    // publishing every vendor-specific message at 200 Hz is intentionally
    // optional to keep CPU and DDS load low on embedded computers.
    if(!publish_extended_topics)
    {
        return;
    }
